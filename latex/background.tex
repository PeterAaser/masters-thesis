...
% This gives latex error about missing $, but upon inserting text is no longer
% bold
% \subsubsection{(DS)^2}
...
turn shapes the further evolution of structure. \unsure{Say something about
  phase space?}
...
evolve over time as a response to perturbation\footnote{Which is why homeopathy
  is considered quackery}. \unsure{At some level of detail, timescale and
  definition of structure water probably can be a $(DS)^2$, but this is not the
  case for the bucket.}
\info{TODO: Odd rune salt experiment. Stepney om tilnærming tilnærming til bio
  fra EiM enten her eller neste section}
